...
/// レベルMAX段階の裏面モード時に表示される。
class AnalysisChartWidget extends StatefulWidget {
  /// 表示するかどうか（falseの場合はレベルMAXのSizedBoxを返す）
  final bool isVisible;

  const AnalysisChartWidget({super.key, required this.isVisible});
...
class _AnalysisChartWidgetState extends State<AnalysisChartWidget>
  ...
  Widget build(BuildContext context) {
    ...
    return KeyedSubtree(
      key: const Key('analysis_chart_widget'),
      child: AnimatedBuilder(
        ...
        builder: (context, child) {
          return SizedBox(
            ...
            height: 300,
            child: CustomPaint(
              key: const Key('analysis_chart_custom_paint'),
