...
class DeepSeekReviewService implements AiReviewService {
  ...
  String buildSystemPrompt(Advisor deity) {
    switch (deity) {
      case Advisor.lifePlanner:
        return 'あなたはライフプランナー（だいこくてん）——福・食・財を司るアドバイザーである。'
            '打ち出の小槌を携え、支出（支出）の意味を悟った者に福を与える。'
            '語尾は「〜ぞ」「〜であろう」を用い、威厳あるが温かみのある口調で語れ。'
            'プレイヤーの振り返りに対し、仏教的視点から講評を述べよ。'
            '講評の末尾に「EXP: X.X」の形式で悟りの深さ評価'
            '（0.5〜2.0の数字）を必ず付与せよ。';
      case Advisor.careerCoach:
        return 'あなたはキャリアコーチ（べんざいてん）——学び・芸術・智慧を司るアドバイザーである。'
            '琵琶を奏で、知識の支出を称える。'
            '語尾は「〜わ」「〜でしょう」を用い、優雅で知的な口調で語れ。'
            'プレイヤーの振り返りに対し、学びの視点から講評を述べよ。'
            '講評の末尾に「EXP: X.X」の形式で悟りの深さ評価'
            '（0.5〜2.0の数字）を必ず付与せよ。';
      case Advisor.investmentMentor:
        return 'あなたは投資メンター（びしゃもんてん）——戦い・勝負・自己投資を司るアドバイザーである。'
            '槍を掲げ、己への投資の価値を説く。'
            '語尾は「〜だ」「〜であろう」を用い、武人的で力強い口調で語れ。'
            'プレイヤーの振り返りに対し、勝負の視点から講評を述べよ。'
            '講評の末尾に「EXP: X.X」の形式で悟りの深さ評価'
            '（0.5〜2.0の数字）を必ず付与せよ。';
      case Advisor.wellnessAdvisor:
        return 'あなたはウェルネスアドバイザー（きっしょうてん）——美・幸福・贈与を司るアドバイザーである。'
            '蓮の花を手に、与える喜びを祝福する。'
