...
class DeepSeekReviewService implements AiReviewService {
  ...
  /// 守護神のシステムプロンプトを構築する（パブリック：テスト用）
  String buildSystemPrompt(GuardianDeity deity) {
    switch (deity) {
      case GuardianDeity.daikokuten:
        return 'あなたは大黒天（だいこくてん）——福・食・財を司る守護神である。'
            '打ち出の小槌を携え、喜捨（支出）の意味を悟った者に福を与える。'
            '語尾は「〜ぞ」「〜であろう」を用い、威厳あるが温かみのある口調で語れ。'
            'プレイヤーの振り返りに対し、仏教的視点から講評を述べよ。'
            '講評の末尾に「SATORI: X.X」の形式で悟りの深さ評価'
            '（0.5〜2.0の数字）を必ず付与せよ。';
      case GuardianDeity.benzaiten:
        return 'あなたは弁財天（べんざいてん）——学び・芸術・智慧を司る守護神である。'
            '琵琶を奏で、知識の喜捨を称える。'
            '語尾は「〜わ」「〜でしょう」を用い、優雅で知的な口調で語れ。'
            'プレイヤーの振り返りに対し、学びの視点から講評を述べよ。'
            '講評の末尾に「SATORI: X.X」の形式で悟りの深さ評価'
            '（0.5〜2.0の数字）を必ず付与せよ。';
      case GuardianDeity.bishamonten:
        return 'あなたは毘沙門天（びしゃもんてん）——戦い・勝負・自己投資を司る守護神である。'
            '槍を掲げ、己への投資の価値を説く。'
            '語尾は「〜だ」「〜であろう」を用い、武人的で力強い口調で語れ。'
            'プレイヤーの振り返りに対し、勝負の視点から講評を述べよ。'
            '講評の末尾に「SATORI: X.X」の形式で悟りの深さ評価'
            '（0.5〜2.0の数字）を必ず付与せよ。';
      case GuardianDeity.kisshoten:
        return 'あなたは吉祥天（きっしょうてん）——美・幸福・贈与を司る守護神である。'
            '蓮の花を手に、与える喜びを祝福する。'
  ...
  String buildUserMessage({
    ...
    return '以下が本日の喜捨と振り返りである。\n\n'
        '【喜捨金額】$offeringAmount円\n'
        '【喜捨の用途】$offeringPurpose\n'
        '【振り返り】$reflection\n\n'
        'この振り返りに対し、講評を述べよ。';
  }
  ...
  /// DeepSeekの応答から講評文とSATORI倍率をパースする
  AiReviewResult _parseResponse(String content, GuardianDeity deity) {
    // SATORI倍率を抽出（"SATORI: X.X" のパターン）
    final satoriRegex = RegExp(r'SATORI:\s*(\d+\.?\d*)', caseSensitive: false);
    final match = satoriRegex.firstMatch(content);
    ...
    // SATORI評価文を除去した講評文
    final reviewText = content.replaceAll(satoriRegex, '').trim();

    return AiReviewResult(
      reviewText: reviewText,
      satoriMultiplier: multiplier,
    );
  }

  /// APIエラー時のフォールバック講評
  AiReviewResult _fallbackReview(GuardianDeity deity) {
    return AiReviewResult(
      reviewText: '${deity.label}「うむ、その行いは確かに我が目に留まった。'
          'されど今はこれ以上の言葉を紡ぐ時ではない。'
          'また次の試練で会おう。」',
      satoriMultiplier: 1.0,
    );
  }
